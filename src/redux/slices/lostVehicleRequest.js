...
import {
  LOST_VEHICLE_REQUEST_ENDPOINT,
  LIST_PAGINATED_MY_LOST_VEHICLE_REQUESTS_ENDPOINT,
...
export function getLostVehicleRequest(params) {
  return async () => {
    ...
    try {
      const response = await axios.get(LIST_PAGINATED_MY_LOST_VEHICLE_REQUESTS_ENDPOINT, {
        params,
      });
...
export function createPostVehicleRequest(params) {
  return async () => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await axios.post(LOST_VEHICLE_REQUEST_ENDPOINT, params);
